...
def load_data(dir_data):
    ...
    tmean_season_std = tmean_season_std["daily_t2_min"]
    return(anomaly,standard_anomaly,tmean_season_std)
...
def process_time_slice_and_plot(time_slice, anom_slice,stdev_slice,time_coord, lat_vals, lon_vals, size_threshold=1750):
    ...
    if np.size(largest_region_label) > 0:
        ...
        if len(region_sizes) > 1:
            ...
            largest_region_size = region_sizes[largest_region_label]
        #largest_region_label = largest_region_label[0] 
...
if __name__ == "__main__":
    ...
    anomaly,standard_anomaly,tmean_season_std = load_data(dir_data)
    
    cao_bool = standard_anomaly.where(tmean_season_std.sel(season='DJF') > 3,0)
    cao_bool = cao_bool < -1.5
    ...
    # Loop through time and process each time slice
        ...
        pd.DataFrame(my_list,columns=["time","lat","lon","min_rel","min_all","anom_mean","size"]).to_csv(os.path.join(dir_out,"tables/{}.csv".format(year)), sep=';',index=False)
    client.close()